...
class CNN(object):
    def __init__(self):
        ...
        """
        self.x = keras.models.Sequential()
        self.weights = []
        self.biases = []
        self.activations = []
        self.loss = None
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        # self.layer1 = tf.random.normal(self.input_shape)
        # self.y = tf.keras.layers.Add()([self.layer1])

        #https://www.tensorflow.org/api_docs/python/tf/keras/layers/InputLayer
        # self.input = keras.layers.InputLayer(
        #     input_shape=shape, batch_size=None, dtype=None, input_tensor=None, name=name
        # )
        self.x.add(keras.layers.InputLayer(input_shape=shape, name=name))
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        ...
        self.x.add(keras.layers.Dense(num_nodes, activation=activation, name=name, trainable=trainable))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         """
        activation = activation.lower()
        self.layer2d = keras.layers.Conv2D(
            num_of_filters, kernel_size=kernel_size, strides=strides, padding=padding, activation=activation, name=name, trainable=trainable
        )
        self.x.add(self.layer2d)
        return self.layer2d
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2, name=""):
        """
         ...
         :return: Layer object
         """
        self.max2d = keras.layers.MaxPool2D(pool_size=pool_size, strides=strides, padding=padding, name=name)
        self.x.add(self.max2d)
        return self.max2d
    ...
    def append_flatten_layer(self,name=""):
        """
         ...
         :return: Layer object
         """
        self.flatten = keras.layers.Flatten(name=name)
        self.x.add(self.flatten)
        return self.flatten
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if isinstance(layer_numbers, int):
            self.x.get_layer(layer_number=layer_numbers, layer_name=layer_names).trainable = trainable_flag
        elif isinstance(layer_numbers, list):
            for layer in layer_numbers:
                self.model.get_layer(layer_number=layer, layer_name=layer_names[layer]).trainable = trainable_flag
        elif isinstance(layer_numbers, str):
            self.model.get_layer(layer_number=layer_numbers, layer_name=layer_names).trainable = trainable_flag
        else:
            for name in layer_names:
                self.model.get_layer(layer_number=layer_numbers, layer_name=layer_names[name]).trainable = trainable_flag
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        if layer_number != None:
            if len(self.x.layers[layer_number - 1].get_weights()) <= 0 or layer_number == 0:
                return None
            elif layer_number == -1:
                return self.x.layers[layer_number].get_weights()[0]
            else:
                return self.x.layers[layer_number - 1].get_weights()[0]
        else:
            if len(self.x.get_layer(name=layer_name).get_weights()) <= 0:
                return None
            else:
                return self.x.get_layer(name=layer_name).get_weights()[0]
    ...
    def get_biases(self, layer_number=None, layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        if layer_number != None:
            if len(self.x.get_layer(index=layer_number - 1).get_weights()) <= 0 or layer_number == 0:
                return None
            elif layer_number == -1:
                return self.x.layers[layer_number].get_weights()[1]
            else:
                return self.x.layers[layer_number - 1].get_weights()[1]
        else:
            if len(self.x.get_layer(name=layer_name).get_weights()) <= 0:
                return None
            else:
                return self.x.get_layer(name=layer_name).get_weights()[1]
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        if layer_number:
            keras.backend.set_value(self.x.get_layer(index=layer_number - 1).weights[0], weights)
        else:
            keras.backend.set_value(self.x.get_layer(name=layer_name).weights[0], weights)
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        """
        if layer_number:
            keras.backend.set_value(self.x.get_layer(index=layer_number - 1).weights[1], biases)
        else:
            keras.backend.set_value(self.x.get_layer(name=layer_name).weights[1], biases)
    ...
    def remove_last_layer(self):
        ...
        """
        self.layer_remove = self.x.pop()

        self.x = keras.models.Sequential(self.x.layers)
        return self.layer_remove
    ...
    def evaluate(self,X,y):
        """
         ...
         """
        return self.x.evaluate(x=X, y=y)
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        """
         ...
         """
        self.x.compile(self.optimizer, loss=self.loss, metrics=[self.metric])

        self.x.fit(x=X_train, y=y_train, batch_size=batch_size, epochs=num_epochs, verbose=1)
